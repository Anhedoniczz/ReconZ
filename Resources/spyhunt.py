...
if args.s:
    if args.save:
        ...
        if not path.exists(f"{args.save}"):
            ...
            sys.exit(1)
        cmd = f"bash ~/tools/spyhunt/scripts/spotter.sh {args.s} | uniq | sort"
        p = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        spotterout, err = p.communicate()
        spotterout = spotterout.decode()
        with open(f"{args.save}", "a") as spotter:
            spotter.writelines(spotterout)
        cmd = f"bash ~/tools/spyhunt/scripts/certsh.sh {args.s} | uniq | sort"
        p = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        ...
        commands(f"bash ~/tools/spyhunt/scripts/spotter.sh {args.s} | uniq | sort")
        commands(f"bash ~/tools/spyhunt/scripts/certsh.sh {args.s} | uniq | sort") 
